...
// Multi-query version of the existing high-accuracy integer FastScan kernel.
// Each packed-code load and nibble extraction is shared by Batch query LUTs.
template<size_t Batch>
inline void accumulate_hacc_queries(
    const uint8_t* __restrict__ codes,
    const uint8_t* const* __restrict__ tables,
    int32_t* accu_res,
    size_t dim
) {
    __m512i low_mask = _mm512_set1_epi8(0xf);
    __m512i accumulators[Batch][2][4];

    for (auto& accu : accumulators) for (auto& a : accu) {
        for (auto& reg : a) {
    ...
    for (size_t m = 0; m < num_codebook; m += 4) {
        ...
        const uint8_t* hc_lut = tables[q] + (m / 4) * 128;
        for (auto& i : accumulators[q]) {
    ...
    _mm512_storeu_epi32(accu_res + q * 32, res[0]);
    _mm512_storeu_epi32(accu_res + q * 32 + 16, res[1]);
    }
}
}
